// generate_ships_v2_json.js — V4 (SCU mapping + optional ships_database merge generator)
// Usage (from project root):
//   node assets/data/generate_ships_v2_json.js
//   node assets/data/generate_ships_v2_json.js --db   (zero-risk: generates ships_database.generated.json)
// Output:
//   assets/data/ships_v2.json
//   assets/data/ships_database.generated.json  (only with --db)
//
// Data sources:
...
// -----------------------------
// Optional mode: generate ships_database.generated.json (zero-risk)
// - Reads local ships_v2.json (generated by this script or provided in repo)
// - Reads local ships_database.json (your curated economics fields)
// - Writes ships_database.generated.json (NEVER overwrites ships_database.json)
// Run:
//   node assets/data/generate_ships_v2_json.js --db
// -----------------------------
const DB_IN = "ships_database.json";
const DB_OUT = "ships_database.generated.json";
...
function pickTypeFromV2(v2){
  // Prefer role (often more specific), then career
  return norm(v2?.role) || norm(v2?.career) || "";
}

function mergeShipsDatabase(){
  const dir = __dirname;

  const v2Path = path.join(dir, "ships_v2.json");
  if(!fs.existsSync(v2Path)){
    throw new Error("ships_v2.json not found. Generate it first, or place it in assets/data/");
  }

  const v2List = JSON.parse(fs.readFileSync(v2Path, "utf-8"));
  if(!Array.isArray(v2List)) throw new Error("ships_v2.json invalid format (expected array).");

  const dbPath = path.join(dir, DB_IN);
  const existingDb = fs.existsSync(dbPath)
    ? JSON.parse(fs.readFileSync(dbPath, "utf-8"))
    : { version: "V1.0.0", pu: "4.5", ships: [] };

  const existingShips = Array.isArray(existingDb?.ships) ? existingDb.ships : [];
  const byKey = new Map(existingShips.map(s => [keyName(s?.name), s]));

  const merged = [];

  for(const v2 of v2List){
    const name = norm(v2?.name);
    if(!name) continue;
    ...
    if(cur){
      // Preserve curated fields; enrich missing from v2
      const out = { ...cur };

      out.name = out.name || name;

      // Manufacturer: fill if missing
      if(!norm(out.manufacturer)) out.manufacturer = norm(v2?.manufacturer);

      // Type: keep if already set, else from v2 role/career
      if(!norm(out.type)) out.type = pickTypeFromV2(v2);

      // SCU: keep if present/non-zero, else from v2
      const curScu = safeNum(out.scu);
      const v2Scu = safeNum(v2?.scu);
      if((curScu === null || curScu === 0) && (v2Scu !== null && v2Scu > 0)){
        out.scu = v2Scu;
      }

      // Keep your current armament / crew / prices / locations as-is
      merged.push(out);
      byKey.delete(k); // consumed
    }else{
      // New ship: populate from v2 with empty curated fields
      merged.push({
        name,
        manufacturer: norm(v2?.manufacturer),
        type: pickTypeFromV2(v2),
        scu: safeNum(v2?.scu) ?? 0,
        armament: "",
        crew_min: null,
        crew_max: null,
        price_uec: null,
  ...
  // Add remaining curated ships that are NOT present in v2 (keep them)
  for(const [, ship] of byKey){
    if(ship?.name) merged.push(ship);
  }

  merged.sort((a,b) => lower(a?.name).localeCompare(lower(b?.name)));

  const outDb = {
    version: "V1.1.0",
    pu: existingDb?.pu || "4.5",
    generated_from: "ships_v2.json",
    generated_at: new Date().toISOString(),
    ships: merged
  };

  const outPath = path.join(dir, DB_OUT);
  fs.writeFileSync(outPath, JSON.stringify(outDb, null, 2), "utf-8");

  const filledPrices = merged.filter(s => safeNum(s?.price_uec) !== null).length;
  console.log(`OK: ${DB_OUT} generated with ${merged.length} ships (${filledPrices} with price_uec)`);
...
(function(){
  ...
  const isDb = args.includes("--db") || args.includes("-db") || args.includes("db");
